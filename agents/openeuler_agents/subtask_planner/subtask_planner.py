...
获取以上信息后，你还需要判断其中已经完成任务的返回结果是否与本次规划有关，如果有关，请你用`ReadJsonFile`读取相应的api或ssh命令的结果文件的内容或用`ReadFile`读取相关文本文件内信息（直接调用工具不要规划为一个step）后再进行规划

注意: 
1. 你每次接收到输入都需要读取一遍context_tree.json
2. 请尽量压缩结构化分析汇总和文本结构化输出等文本相关任务的数量，以尽可能少的任务完成这类目标
3. 尽量避免规划结构化整理信息的任务，报告输出任务可以自己收集信息
...
3. 子任务的内容不能与其他任何任务的内容有重复，避免过度规划,特别是避免规划已经可以确定完成过的任务。
